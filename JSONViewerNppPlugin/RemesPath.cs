...
{
    public class VectorizedArithmeticException : Exception
    {
        public string Message;
        public VectorizedArithmeticException(string message)
        {
            Message = message;
        }
    ...
    /// <summary>
    /// Exception thrown while parsing or traversing RemesPaths.
    /// </summary>
    public class RemesPathException : Exception
    {
        public string Message;

        public RemesPathException(string Message) { this.Message = Message; }
    ...
    public class RemesParser
    {
        ...
        public JNode Compile(string query)
        {
            ...
            // turns out compiling queries is very fast (tens of microseconds for a simple query), so caching old queries doesn't save much time
            return result;
        }
        ...
        public Func<JNode, IEnumerable<(object key, JNode node)>> ApplyMultiIndex(object inds, bool is_varname_list, bool is_recursive = false)
        {
            if (inds is CurJson)
            {
        ...
        public IEnumerable<(string key, JNode val)> ApplyRegexIndex(JObject obj, Regex regex)
        {
        ...
        public Func<JNode, IEnumerable<(object key, JNode node)>> ApplyBooleanIndex(JNode inds)
        {
            IEnumerable<(object key, JNode node)> bool_idxr_func(JNode x)
            {
                ...
                else if (newinds.type == Dtype.OBJ)
                {
                    ...
                    {
                        throw new VectorizedArithmeticException($"Boolean index length {iobj.Length} does not match object/array length {xobj.Length}.");
                    }
                    foreach ((string key, JNode xval) in xobj.children)
                    {
                        ...
                        if (i_has_key)
                        {
                            ...
                            {
                                throw new VectorizedArithmeticException("Boolean index contains non-booleans");
                            }
                            if ((bool)ival.value)
                            {
                ...
                else if (newinds.type == Dtype.ARR)
                {
                    ...
                    {
                        throw new VectorizedArithmeticException($"Boolean index length {iarr.Length} does not match object/array length {xarr.Length}.");
                    }
                    for (int ii = 0; ii < xarr.Length; ii++)
                    {
        ...
        public IEnumerable<(object key, JNode val)> ApplyStarIndexer(JNode x)
        {
            if (x.type == Dtype.OBJ)
            {
                var xobj = (JObject)x;
        ...
        /// <param name="x"></param>
        /// <returns></returns>
        public static bool? ObjectOrArrayEmpty(JNode x)
        {
        ...
        public Func<JNode, JNode> ApplyIndexerList(List<IndexerFunc> indexers)
        {
            JNode idxr_list_func(JNode obj, List<IndexerFunc> indexers)
            {
                ...
                if (!inds.MoveNext())
                {
                    ...
                    return ix.is_dict ? new JObject(0, new Dictionary<string, JNode>()) : new JArray(0, new List<JNode>());
                }
        ...
        public JNode BinopTwoJsons(Binop b, JNode left, JNode right)
        {
        ...
        public JNode BinopJsonScalar(Binop b, JNode left, JNode right)
        {
        ...
        public JNode BinopScalarJson(Binop b, JNode left, JNode right)
        {
            if (right.type == Dtype.OBJ)
            {
                var dic = new Dictionary<string, JNode>();
        ...
        public Dtype BinopOutType(Binop b, Dtype ltype, Dtype rtype)
        {
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        public JNode ResolveBinop(Binop b, JNode left, JNode right)
        {
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        public JNode ResolveBinopTree(BinopWithArgs b)
        {
            object left = b.left;
        ...
        public JNode ApplyArgFunction(ArgFunctionWithArgs func)
        {
            JNode x = func.args[0];
        ...
        public object? PeekNextToken(List<object> toks, int ii)
        {
            if (ii + 1 >= toks.Count) { return null; }
            return toks[ii + 1];
        }

        public (JSlicer, int ii) ParseSlicer(List<object> toks, int ii, int? first_num)
        {
            ...
            while (ii < toks.Count)
            {
                ...
                try
                {
                    (JNode numtok, ii) = ParseExprOrScalarFunc(toks, ii);
        ...
        public object GetSingleIndexerListValue(JNode ind)
        {
            switch (ind.type)
            {
        ...
        public (Indexer indexer, int ii) ParseIndexer(List<object> toks, int ii)
        {
            object t = toks[ii];
            object? nt;
            if (t is not char)
            ...
            if (d == '.')
            {
                ...
                if (nt != null)
                {
                    ...
                    children.Add((jnt is JRegex) ? ((JRegex)jnt).regex : jnt.value);
        ...
        public (JNode node, int ii) ParseExprOrScalar(List<object> toks, int ii)
        {
            if (toks.Count == 0)
            {
            ...
            if ((last_tok.type & Dtype.ITERABLE) != 0)
            {
                ...
                while (nt != null && nt is char && INDEXER_STARTERS.Contains((char)nt))
                {
                    ...
                    (Indexer cur_idxr, ii) = ParseIndexer(toks, ii);
        ...
        public (JNode node, int ii) ParseExprOrScalarFunc(List<object> toks, int ii)
        {
            object? curtok = null;
        ...
        public (JNode node, int ii) ParseArgFunction(List<object> toks, int ii, ArgFunction fun)
        {
            object t = toks[ii];
            if (!(t is char && (char)t == '('))
            {
            ...
            while (ii < toks.Count)
            {
                ...
                try
                {
                    ...
                    if ((Dtype.SLICE & type_options) != 0)
                    {
                        ...
                        if (nt is char && (char)nt == ':')
                        {
                            int? first_num = cur_arg == null ? null : Convert.ToInt32(cur_arg.value);
        ...
        public (Indexer proj, int ii) ParseProjection(List<object> toks, int ii)
        {
            var children = new List<(object, JNode)>();
            bool is_object_proj = false;
            while (ii < toks.Count)
            {
                (JNode key, ii) = ParseExprOrScalarFunc(toks, ii);
                object? nt = PeekNextToken(toks, ii - 1);
                if (nt is char)
                {
                    char nd = (char)nt;
                    if (nd == ':')
                    {
                        ...
                        {
                            (JNode val, ii) = ParseExprOrScalarFunc(toks, ii + 1);
    ...
    public class RemesParserTester
    {
        public static void Test()
        {
            ...
            var testcases = new (string query, string desired_result)[]
            {
                ...
                ("irange(2, len(@)) * 3", "[6, 9, 12, 15, 18, 21]"),
    ...
    public class RemesPathBenchmarker
    {
        ...
        {
            (JNode json, int len, long[] load_times) = LoadJsonAndTime(@"C:\Users\mjols\Documents\csharp\JSON_Viewer_cmd\big_random.json", num_trials);
