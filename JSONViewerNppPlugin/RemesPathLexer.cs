    ...
    public class RemesPathLexer
    {
        ...
        public static List<object> Tokenize(string q)
        {
            ...
            while (ii < q.Length)
            {
                ...
                {
                    (object curtok, ii) = ParseNumber(q, ii-1);
                    tokens.Add(curtok);
                }
    ...
    class RemesPathLexerTester
    {
        public static void Test()
        {
            ...
            var testcases = new (string input, List<object?> desired, string msg)[]
            {
                ...
                ("== in =~", new List<object?>(new object?[]{Binop.BINOPS["=="], Binop.BINOPS["in"], Binop.BINOPS["=~"]}), "two-character binops"),
                ...
                ("2 <=3", new List<object?>(new object?[]{(long)2, Binop.BINOPS["<="], (long)3}), "binop where a substring is also a binop")
            };
            int tests_failed = 0;
            int ii = 0;
            foreach ((string input, List<object?> desired, string msg) in testcases)
            {
                ...
                {
                    if (value is JNode && value is not CurJson)
                    {
