...
namespace JSON_Viewer.JSONViewerNppPlugin
{
    ...
    public class JsonParser
    {
            ...
            if (mtch.Success)
            {
                try
                {
                    if (len == 10)
                    {
                        ...
                        return new JNode(DateOnly.Parse(maybe_datetime), Dtype.DATE, line_num);
                    }
                    if (len >= 19 && len <= 23)
                    {
        ...
        public (JNode str, int pos, int line_num) ParseString(string inp, int startpos, int line_num, char quote_char = '"')
        {
            ...
            while (true)
            {
                ...
                if (cur_c == '\n')
                {
                    ...
                    lint.Add(new JsonLint($"String literal terminated by newline starting at position {startpos}", startpos, line_num, inp[startpos]));
                    break;
                }
                if (cur_c == quote_char)
                {
        ...
        public (JNode num, int ii, int line_num) ParseNumber(string q, int ii, int line_num)
        {
            ...
            while (ii < q.Length)
            {
                ...
                else if (c == '/')
                {
                    ...
                    (JNode denom_node, ii, line_num) = ParseNumber(q, ii + 1, line_num);
        ...
        public (JArray arr, int pos, int line_num) ParseArray(string inp, int startpos, int line_num)
        {
            ...
            while (ii < inp.Length)
            {
                ...
                else
                {
                    ...
                    (JNode new_obj, ii, line_num) = ParseSomething(inp, ii, line_num);
        ...
        public (JObject obj, int pos, int line_num) ParseObject(string inp, int startpos, int line_num)
        {
            ...
            while (ii < inp.Length)
            {
                    ...
                    )
                {
                    ...
                    (JNode keystring, ii, line_num) = ParseString(inp, ii, line_num, cur_c);
        ...
        public (JNode node, int pos, int line_num) ParseSomething(string inp, int ii, int line_num)
        {
            ...
            char cur_c = inp[ii];
            char next_c;
            if (cur_c == '"' || (allow_singlequoted_str && cur_c == '\''))
            {
        ...
        public JNode Parse(string inp)
        {
            ...
            (JNode node, _, _) = ParseSomething(inp, startpos, line_num);
            return node;
        }
    }
        #endregion
    ...
    public class JsonParserTester
    {
        public static void Test()
        {
            ...
            {
                Tuple.Create(example, norm_example, pprint_example, "general parsing"),
                Tuple.Create("1/2", "0.5", "0.5", "fractions"),
                Tuple.Create("[[]]", "[[]]", "[\n    [\n    ]\n]", "empty lists"),
                Tuple.Create("\"abc\"", "\"abc\"", "\"abc\"", "scalar string"),
                Tuple.Create("1", "1", "1", "scalar int"),
                Tuple.Create("-1.0", "-1.0", "-1.0", "negative scalar float"),
                Tuple.Create("3.5", "3.5", "3.5", "scalar float"),
                Tuple.Create("-4", "-4", "-4", "negative scalar int"),
                Tuple.Create("[{\"FullName\":\"C:\\\\123467756\\\\Z\",\"LastWriteTimeUtc\":\"\\/Date(1600790697130)\\/\"}," +
...
2]",
                ...
                ("\"2022-06-04\"", new JNode(new DateOnly(2022, 6, 4), Dtype.DATE, 0)),
                ...
                new JArray(0, new List<JNode>(new JNode[]{new JNode("abc", Dtype.STR, 0), 
                                                          ...
                                                          new JNode(new DateOnly(1999, 1, 3), Dtype.DATE, 0)}))),
        ...
        public static void TestLinter()
        {
            ...
            var testcases = new (string inp, string desired_out, string[] expected_lint)[]
            {
                ...
                ("[1, \"a\n]", "[1, \"a\"]", new string[]{$"String literal terminated by newline starting at position 4"}),
                ("[1  \"a\n]", "[1, \"a\"]", new string[]{"No comma between array members", "String literal terminated by newline starting at position 4"}),
            };
