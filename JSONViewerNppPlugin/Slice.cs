    ...
    public static class SliceExtensions
    {
        ...
        // come with the Slice method built in.
        {
            ...
            switch (parts.Length)
            {
                ...
                case 2:
                start = parts[0].Equals("") ? null : int.Parse(parts[0]);
                stop = parts[1].Equals("") ? null : int.Parse(parts[1]);
                break;
                case 3:
                start = parts[0].Equals("") ? null : int.Parse(parts[0]);
                stop = parts[1].Equals("") ? null : int.Parse(parts[1]);
                stride = parts[2].Equals("") ? null : int.Parse(parts[2]);
                break;
            }
        ...
        public static IList<T> Slice<T>(this IList<T> source, int? start, int? stop = null, int? stride = null)
        {
            ...
            if (len <= 0)
            {
                return isarr ? new T[0] : new List<T>();
            }
            int istart, istop, istride;
            if (start != null && stop == null && stride == null)
            {
